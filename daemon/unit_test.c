                                    ...
                                    int current_region, time_t time_start, time_t time_end)
{
    ...
    for (i = 0 ; i < CHARTS ; ++i) {
        RRDR *r = rrd2rrdr(st[i], points, time_start + update_every, time_end, RRDR_GROUPING_AVERAGE, 0, 0, 0, NULL, NULL);
        if (!r) {
...
int test_dbengine(void)
{
    ...
    for (i = 0 ; i < CHARTS ; ++i) {
        RRDR *r = rrd2rrdr(st[i], points, time_start[0] + update_every, time_end[REGIONS - 1], RRDR_GROUPING_AVERAGE, 0, 0, 0, NULL, NULL);
        if (!r) {
            fprintf(stderr, "    DB-engine unittest %s: empty RRDR ### E R R O R ###\n", st[i]->name);
            ++errors;
        } else {
